...
- **Pin slide height to the real header.** Measure the sticky header into `--header-h` via JS (on load, resize, font-load, and mode toggle), not a magic number. Slides are `min-height:calc(100vh - var(--header-h))`, and *every* slide including the hero snaps with `scroll-margin-top:var(--header-h)`. Omit the hero's margin and it snaps a header-height too low, leaking a sliver of the next slide that proximity-snap keeps pulling back to.
...
- **Presenter Mode strips page chrome.** It hides the TOC nav and the site-back button (`#site-back-btn`), and re-measures the header into `--header-h` (the nav bar is gone, so it's shorter) so no dead band or sliver appears. A deck shows content, not navigation.
